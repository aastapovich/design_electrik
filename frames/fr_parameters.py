...
def FrameParameters(parent, saved_data):
    """ Фрейм с параметрами. """

    frame_parameters = ttk.Frame(parent, relief=tk.SUNKEN, borderwidth=1)
    frame_parameters.grid(row=0, column=0, sticky='nsew')
    ...
    ttk.Label(frame_parameters, text="Количество проводников").pack(anchor='w', padx=10, pady=(5, 0))
    ...
    conductors.pack(anchor='w', fill=tk.X, padx=20, pady=2)
    conductors.bind("<<ComboboxSelected>>", lambda e: update_saved_data("Количество проводников", conductors))

    # Тип монтажа (Combobox)
    ttk.Label(frame_parameters, text="Тип монтажа").pack(anchor='w', padx=10, pady=(5, 0))
    ...
    mount_type.pack(anchor='w', fill=tk.X, padx=20, pady=2)
    mount_type.bind("<<ComboboxSelected>>", lambda e: update_saved_data("Тип монтажа", mount_type))

    # Варианты компоновки (Combobox)
    ttk.Label(frame_parameters, text="Варианты компоновки").pack(anchor='w', padx=10, pady=(5, 0))
    ...
    layout_options.pack(anchor='w', fill=tk.X, padx=20, pady=2)
