...
def FrameArchitecture(parent, saved_data):
    """ Фрейм с архитектурными данными. """

    frame_arhitecture = ttk.Frame(parent, relief=tk.SUNKEN, borderwidth=1)
    frame_arhitecture.grid(row=0, column=0, sticky='nsew')
    ...
    wall_material = ttk.Combobox(frame_arhitecture, values=[
                                                    "Кирпич черный полнотелый",
                                                    "Кирпич черный пустотелый",
    ...
    ceiling_finish = ttk.Combobox(frame_arhitecture, values=[
        ...
        "Дерево, ОСП по брускам", "Шпаклевка, покраска"
        ])
